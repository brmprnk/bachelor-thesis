...
class MMVAE(nn.Module):
    ...
    def reconstruct_sample(self, data):
        self.eval()
        cross_modal = [[self.vaes[v] for v in range(len(self.vaes))] for _ in range(len(self.vaes))]
        # print("Cross modal matrix", cross_modal)
        with torch.no_grad():
            _, px_zs, _ = self.forward(data)
            # print("px_zs", px_zs)
            # print(len(px_zs))
            # for vae_list in px_zs:
                # print("vae_list", vae_list)
                # for px_z in vae_list:
                    # print("px_z", px_z)
                    # print("sample from px_z", px_z.sample())
