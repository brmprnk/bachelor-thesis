#RNA-GCN-DNA multi-modal model specification
...
from .vae_dna import DNA

scale_factor = 10000
modal = ['rna', 'gcn', 'dna']


class RNA_GCN_DNA(MMVAE):
    def __init__(self, params):
        prior = dist.Laplace
        super(RNA_GCN_DNA, self).__init__(prior, params, RNA, GCN, DNA)
    ...
    def reconstruct(self, data, sampling=False, N=1):
        #vmin = 0
        if not sampling:
            recons_mat = super(RNA_GCN_DNA, self).reconstruct(data)
            return recons_mat
            ...
            for n in range(N):
                recons_mat = super(RNA_GCN_DNA, self).reconstruct_sample(data)
                for r, recons_list in enumerate(recons_mat):
                    for o, recon in enumerate(recons_list):
                        _data = data[r].cpu()
                        recon = recon.squeeze(0).cpu().detach().numpy()
    ...
    def analyse(self, data, runPath, epoch, K=1):
        zemb, zsl, kls_df = super(RNA_GCN_DNA, self).analyse(data, K=K)
    ...
    def get_latent(self, data, train_test, runPath, sampling=False):
        lats = super(RNA_GCN_DNA, self).latents(data, sampling)
    ...
    def plot_klds(self, data, runPath):
        kls_df = super(RNA_GCN_DNA, self).kls_df(data)
